...
bool is_closing_executable_file(struct file *file);
bool is_opening_executable_file(const char *file);
...
/* 문자열의 유효성 검사 함수 */
void check_valid_string(const char *str) {
    const char *current_char = str;               // 현재 문자 포인터
    check_valid_vaddr((const void *)current_char); // 첫 번째 문자 주소 유효성 검사
    
    while (*current_char) {                        // 문자열 끝까지 반복
        check_valid_vaddr((const void *)current_char); // 각 문자의 주소 유효성 검사
        current_char++;                            // 다음 문자로 이동
    }
}
...
bool is_closing_executable_file(struct file *file) {
    ...
    return (file == cur->exec_file);
}

bool is_opening_executable_file(const char *file) {
     struct thread *cur = thread_current();
    return (strcmp(file, cur->name) == 0);
}
...
struct file *validate_fd(int fd) {
    ...
    /* 유효x-1 fd가 0 또는 1인 경우는 유효하지 않음 */
    if (fd == 0 || fd == 1) {
        exit(-1);
    }

    /* 유효x-2 fd가 2 이상이고, 현재 스레드의 next_fd 미만이며, fdt[fd]가 NULL이 아닌 경우 */
...
int open(const char *file)
{
  ...
  struct thread *cur = thread_current();
//printf("Debug: exec_file=%p, opened_file=%p\n", (void *)cur->exec_file, (void *)opened_file);

//  printf("open(): filesys_open()이 파일 '%s'에 대해 성공함\n", file);
  //  printf("open(): 파일 '%s'을(를) 스레드 이름 '%s'과(와) 비교 중\n", file, cur->name);
 // 파일 이름을 비교하여 실행 파일인지 확인
if (is_opening_executable_file(file)) 
 {
...
struct file *file = validate_fd(fd); 
 struct thread *cur = thread_current();
  ...
  // 실행 파일인지 확인
  // file이랑 cur->fdt[fd]랑 동일함 
 if (is_closing_executable_file(file)) 
  {
...
int filesize(int fd)
{
  ...
  struct file *file = validate_fd(fd);  // 파일 디스크립터 검증 및 파일 포인터 반환
